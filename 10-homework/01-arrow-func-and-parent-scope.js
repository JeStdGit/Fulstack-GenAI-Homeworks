...
// locked down and cannot be changed (that why `user.greetArrow.bind(user).call()` won't work...).
// callbacks don't save the call reference (who call this function) and call the functions from the global scope, so when you
...
// written - `user` but objects do not define a scope, so it goes up to the global scope and it becomes itss context
